...
rm *.err *.nav *.snm *.out *.vrb *.py *.dvi *.ps  *.aux */*.aux *.log *.toc *.mtc* *.synctex.gz *.ptc *.idx *.maf *.bbl *.thm *.dep *.gz *.listing *concordance.tex  *.pyg &> /dev/null
...
mkdir ./image
 ...
 find . -type f -name "*.eps" -exec mv -f {} ./image \;
